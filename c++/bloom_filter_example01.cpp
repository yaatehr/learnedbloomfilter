...
public:
   ...
   void SetUp(const ::benchmark::State &state)
   {
      ...
      if(valid_string_map.count(key) == 0) {
         ...
         // std::cout << "starting size is" << vec.size() << std::endl;

         std::shared_ptr<std::vector<std::string>> s = std::make_shared<std::vector<std::string>>(vec);
         std::shared_ptr<std::vector<std::string>> i = std::make_shared<std::vector<std::string>>(gen_string_array(state.range(2) * 5, state.range(3), "#"));
...
public:
   ...
   void SetUp(const ::benchmark::State &state)
   {
      ...
      std::string key = "k" + std::to_string(state.range(2)) + std::to_string(state.range(3));
      if(valid_string_map.count(key) == 0) {
         auto vec = gen_string_array(state.range(2), state.range(3), true);
         std::shared_ptr<std::vector<std::string>> s = std::make_shared<std::vector<std::string>>(vec);
         std::shared_ptr<std::vector<std::string>> i = std::make_shared<std::vector<std::string>>(gen_string_array(state.range(2) * 5, state.range(3), false));
