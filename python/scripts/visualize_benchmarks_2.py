...
viable_tests = ["timestamp_lstm_3", "explicit_gru_depth16", "explicit_gru_depth16_lowfpr", "explicit_gru_h32_lowfpr", "explicit_lstm_depth16", "timestamp_embedding_lstm_4", "timestamp_gru_h32", "timestamp_lstm_h32", "timestamp_lstm_h64"]


for test_name in viable_tests:
    # test_name = "explicit_always_false"
    # test_name = "timestamp_lstm_3"
    # test_name = "explicit_gru_depth16"
    # test_name = "explicit_gru_depth16_lowfpr"
    # test_name = "explicit_gru_h32_lowfpr"
    ...
    num_eles_tested = 13317 if "explicit" in test_name else 50000

    fixture_data["ns_per_insert"] = fixture_data["insert_time"]/num_eles_tested
    fixture_data["ns_per_query"] = fixture_data["query_time"]/num_eles_tested

    fixture_data["empirical_fpr"] = fixture_data["empirical_fpr"] *50000 / num_eles_tested
    full_size_fixture_data["empirical_fpr"] = full_size_fixture_data["empirical_fpr"] *50000 / num_eles_tested
    ...
    for ind, query in enumerate(queries): 
        ...
        for group1_label, group1 in query.groupby("tau"):
            ...
            # print(g.tail(10))
            # print(fg.tail(10))
            abbrev_label = "%.1f" % (float(group1_label)*100)
            fig, ax = plt.subplots()
